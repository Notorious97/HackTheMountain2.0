...
let transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "sakshamanand150@gmail.com",
    pass: "saksham",
  },
});
...
module.exports.getallfiles = async (req, res) => {
  ...
  const pdf = [];
  try {
    const studentUpload = await Studentupload.find({
      teacherID: req.params.id,
    });
    Object.values(studentUpload).forEach(
      ({ name, subject, teacherID, studentID, pdfID, pdffilename }) => {
        pdfname.push({
          name,
          subject,
          teacherID,
