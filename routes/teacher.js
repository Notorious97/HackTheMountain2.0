...
const client = require("filestack-js").init("AIpDyAjyETlSCtxLVaGjCz");
...
router.get("/teacher-home/:teacher_id", async (req, res) => {
  Teacher.findOne({ _id: req.params.teacher_id })
    .populate("createdClassrooms")
    .exec((err, foundTeacher) => {
...
router.get("/view-classroom/:uniqueCode/:teacher_id", async (req, res) => {
  Classroom.findOne({ uniqueCode: req.params.uniqueCode })
    .populate("assignments")
    .exec((err, foundClassroom) => {
      if (err) return res.send("something went wrong");
      Classroom.findById(foundClassroom._id)
        .populate("studentsEnrolled")
        .exec((err, newClassroom) => {
          return res.render("teacherAssignment", {
            classroom: foundClassroom,
            students: newClassroom.studentsEnrolled,
...
router.post(
  "/create-assignment/:classroom_id/:teacher_id",
  upload.single("pdfLink"),
  async (req, res) => {
    var pdfUrl;
    ...
    await Classroom.findOne({ _id: req.params.classroom_id })
      .populate("assignments")
      .exec((err, foundClassroom) => {
        if (err) return res.send("something went wrong!");
...
router.get(
  "/viewSubmissions/:classroom_id/:assignment_id/:teacher_id",
  (req, res) => {
    Classroom.findById(req.params.classroom_id)
      .populate("assignments")
      .exec(async (err, foundClassroom) => {
        if (err) {
          console.error(err);
          res.redirect("back");
        } else {
          ...
          return res.render("teachersubmission", {
            array: array,
