...
router.post(
  "/submit-answer/:assignment_id/:student_id",
  upload.single("pdf"),
  async (req, res) => {
    Assignment.findById(
      req.params.assignment_id,
      async (err, foundAssignment) => {
        if (err) return res.send("something went wrong");
...
router.get(
  "/view-classroom-by-student/:classroom_id/:student_id",
  async (req, res) => {
    Classroom.findOne({ _id: req.params.classroom_id })
      ...
      .exec((err, foundClassroom) => {
        if (err) return res.send("something went wrong");
        Student.findById(req.params.student_id, (err, foundStudent) => {
          return res.render("studentAssignment", {
